...
public class PracticeWritingReview extends AppCompatActivity {
    public static final String ID_TEST_RECORD_KEY = "id_test_record_key";

    PracticeWriting rawReview;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_practice_writing_review);
        ...
        if(Utils.isLoggedIn(this)){
            Gson gson = new Gson();
            rawReview = gson.fromJson( getIntent().getStringExtra("record"), PracticeWriting.class );
        }else{
            long id = this.getIntent().getExtras().getLong(ID_TEST_RECORD_KEY);
            rawReview = PracticeWriting.getPracticeWritingById((int)id);
        }

        writingReviews = rawReview.getReviews();
        changeColorBtnQuestion();

        addFragment(new int[]{1}, new WritingReviewFragment(writingReviews.get(0), listBtnQuestion.get(0)));
        currentQuestion = 1;
    ...
    private void changeColorBtnQuestion(){
        int index = 0;
        for(WritingReview writingReview: writingReviews){
    ...
    private void nextQuestion(){
        ...
        addFragment(new int[]{currentQuestion+1}, new WritingReviewFragment(writingReviews.get(currentQuestion+1-1), listBtnQuestion.get(currentQuestion+1-1)));
        currentQuestion++;
    ...
    public void btnQuestionClick(View view){
        ...
        addFragment(new int[]{number}, new WritingReviewFragment(writingReviews.get(number-1), listBtnQuestion.get(number-1)));
